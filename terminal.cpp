...
Terminal::~Terminal() {
    ...
    if (piProcInfo.hThread != NULL) CloseHandle(piProcInfo.hThread);
    if (siStartupInfo.lpAttributeList != NULL) HeapFree(GetProcessHeap(), 0, siStartupInfo.lpAttributeList);
#else
    if (ptyFd >= 0) {
        close(ptyFd);
    }
...
void Terminal::startShell() {
    ...
    COORD coordSize = { (short)state.col, (short)state.row };
    HRESULT hr = CreatePseudoConsole(coordSize, hInputRead, hOutputWrite, 0, &hPc);
    if (FAILED(hr)) {
        // Error (ConPTY not supported?)
        return;
    }

    // Initialize extended startup info for ConPTY
    SIZE_T attrListSize = 0;
    siStartupInfo.StartupInfo.cb = sizeof(STARTUPINFOEX);
    InitializeProcThreadAttributeList(NULL, 1, 0, &attrListSize);
    siStartupInfo.lpAttributeList = reinterpret_cast<LPPROC_THREAD_ATTRIBUTE_LIST>(HeapAlloc(GetProcessHeap(), 0, attrListSize));
...
// Standard local modes for interactive shells
...
#else // Logic for Linux and other Unix-like systems (also launch as login shell)
        ...
        fprintf(stderr, "FATAL: Failed to execv shell '%s' (intended argv[0]='%s'): %s\n", shell_path_buf, new_argv_linux[0] ? new_argv_linux[0] : "(null)", strerror(errno));
    ...
    // commands. This is a heuristic; 100ms is a common value.
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
    std::string command_str = std::string("cd ~\nclear\n");
...
void Terminal::handleTerminalResize() {
    ...
    float charWidth = ImGui::GetFont()->GetCharAdvance('M');
    float lineHeight = ImGui::GetTextLineHeight();
    int new_cols = MAX(1, static_cast<int>(contentSize.x / charWidth));
    int new_rows = MAX(1, static_cast<int>(contentSize.y / lineHeight));
...
void Terminal::handleGlyphColors(const Glyph & glyph, ImVec4 & fg, ImVec4 & bg) {
    ...
    if (glyph.mode & ATTR_BOLD && glyph.colorMode == COLOR_BASIC) {
        fg.x = MIN(1.0f, fg.x * 1.5f);
        fg.y = MIN(1.0f, fg.y * 1.5f);
        fg.z = MIN(1.0f, fg.z * 1.5f);
    }
}
    ...
    if (state.mode & MODE_INSERT) {
        ...
        if (cursorCell.u != 0) {
            ...
            drawList->AddRectFilled(
                cursorPos, ImVec2(cursorPos.x + charWidth, cursorPos.y + lineHeight),
        ...
        if (screenY >= 0 && screenY < state.row) {
            ...
            for (int x = 0; x < state.col; x++) {
                ...
                if (selectedText(x, selectionY)) {
                    ...
                    drawList->AddRectFilled(
                        highlightPos, ImVec2(highlightPos.x + charWidth, highlightPos.y + lineHeight),
            ...
            if (scrollbackIndex >= 0 && scrollbackIndex < scrollbackBuffer.size()) {
                for (int x = 0; x < state.col; x++) {
                    ...
                    if (selectedText(x, selectionY)) {
                        ImVec2 highlightPos(pos.x + x * charWidth, pos.y + (y - startY) * lineHeight);
                        drawList->AddRectFilled(highlightPos, ImVec2(highlightPos.x + charWidth, highlightPos.y + lineHeight), ImGui::ColorConvertFloat4ToU32(
...
int Terminal::eschandle(unsigned char ascii) 
{
    ...
    case '[':
        state.esc |= ESC_CSI;
        return 0; // Handle O sequence directly for cursor moves
    case 'O':
...
void Terminal::setMode(bool set, int mode) {
    ...
    case 6: // DECOM -- Origin
        ...
        if (set)
            moveTo(0, state.top);
        break;
    case MODE_WRAP: // Line wrapping mode
        break;
    case MODE_INSERT: // Toggle insert mode
        break;
    case MODE_ALTSCREEN:
        if (set) {
        ...
        break;
    case MODE_CRLF: // Change line feed behavior
        break;
    case MODE_ECHO: // Local echo mode
        break;
    }
}
...
void Terminal::reset() {
    ...
    state.charset = 0; // Reset character set translation table
    for (int i = 0; i < 4; i++) {
...
void Terminal::clearRegion(int x1, int y1, int x2, int y2) {
    ...
    x1 = MAX(0, MIN(x1, state.col - 1));
    x2 = MAX(0, MIN(x2, state.col - 1));
    y1 = MAX(0, MIN(y1, state.row - 1));
    y2 = MAX(0, MIN(y2, state.row - 1));
...
void Terminal::moveTo(int x, int y) {
    ...
    // Reset wrap-next state when moving to end of line
    if (state.c.x == state.col - 1) {
        state.c.state |= CURSOR_WRAPNEXT;
    }
}
...
size_t Terminal::utf8Decode(const char* c, Rune * u, size_t clen) {
    *u = UTF_INVALID;
    ...
    if ((c[0] & 0x80) == 0) { // ASCII character
        *u = c[0];
        return 1;
    }
    else if ((c[0] & 0xE0) == 0xC0) { // 2-byte sequence
        len = 2;
        udecoded = c[0] & 0x1F;
    }
    else if ((c[0] & 0xF0) == 0xE0) { // 3-byte sequence (used by box drawing characters)
        len = 3;
        udecoded = c[0] & 0x0F;
    }
    else if ((c[0] & 0xF8) == 0xF0) { // 4-byte sequence
        len = 4;
        udecoded = c[0] & 0x07;
    }
    else {
...
void Terminal::resize(int cols, int rows) {
    ...
    try {
        ...
        int minRows = MIN(rows, state.row);
        int minCols = MIN(cols, state.col);
        ...
        state.top = LIMIT(state.top, 0, rows - 1);
        state.bot = LIMIT(state.bot, state.top, rows - 1);
        if (state.bot < state.top) state.bot = state.top;
        // Swap in new buffers
        ...
        state.c.x = MIN(state.c.x, cols - 1);
        state.c.y = MIN(state.c.y, rows - 1);
...
#ifdef _WIN32
            DWORD written = 0;
            WriteFile(hInputWrite, input.c_str(), input.length(), &written, NULL);
#else
...
#ifdef _WIN32
            DWORD written = 0;
            WriteFile(hInputWrite, "\033OA", 3, &written, NULL);
#else
...
#ifdef _WIN32
            DWORD written = 0;
            WriteFile(hInputWrite, "\033OB", 3, &written, NULL);
#else
...
#ifdef _WIN32
            DWORD written = 0;
            WriteFile(hInputWrite, "\033OC", 3, &written, NULL);
#else
...
#ifdef _WIN32
            DWORD written = 0;
            WriteFile(hInputWrite, "\033OD", 3, &written, NULL);
#else
...
#ifdef _WIN32
        DWORD written = 0;
        WriteFile(hInputWrite, "\r", 1, &written, NULL);
#else
        write(ptyFd, "\r", 1);
#endif
        return;
...
#ifdef _WIN32
        DWORD written = 0;
        WriteFile(hInputWrite, "\b \b", 3, &written, NULL);
#else
        write(ptyFd, "\b \b", 3);
#endif
        return;
...
void Terminal::selectionNormalize() {
    ...
    sel.nb.x = LIMIT(sel.nb.x, 0, state.col - 1);
    sel.ne.x = LIMIT(sel.ne.x, 0, state.col - 1);
...
std::string Terminal::getSelection() {
    ...
    for (int absY = selStartY; absY <= selEndY; absY++) {
        ...
        xstart = LIMIT(xstart, 0, static_cast<int>(line->size()) - 1);
        xend = LIMIT(xend, 0, static_cast<int>(line->size()) - 1);
        for (int x = xstart; x <= xend; x++) {
            if ((*line)[x].mode & ATTR_WDUMMY) continue;
            char buf[UTF_SIZ];
...
#ifdef _WIN32
        DWORD written = 0;
        WriteFile(hInputWrite, "\033[200~", 6, &written, NULL);
#else
        write(ptyFd, "\033[200~", 6);
#endif
        // Send the actual text
#ifdef _WIN32
        WriteFile(hInputWrite, text, strlen(text), &written, NULL);
#else
        write(ptyFd, text, strlen(text));
#endif
        // Send paste end sequence
#ifdef _WIN32
        WriteFile(hInputWrite, "\033[201~", 6, &written, NULL);
#else
...
bool Terminal::selectedText(int x, int y) {
    if (sel.mode == SEL_IDLE || sel.ob.x == -1 || sel.alt != (state.mode & MODE_ALTSCREEN)) return false;
...
void Terminal::handleOSCColor(const std::vector<std::string> &args) {
    ...
    if (args.size() > 2) {
        ...
        if (args[2][0] == '?') {
            ...
            char response[64];
            snprintf(response, sizeof(response), "\033]4;%d;rgb:%.2X/%.2X/%.2X\007",
                index, (int)(state.c.fg.x * 255), (int)(state.c.fg.y * 255), (int)(state.c.fg.z * 255));
...
void Terminal::handleOSCSelection(const std::vector<std::string> &args) {
    ...
    if (args[1] == "c") { // clipboard
        std::string decoded;
        // You would implement base64 decoding
